...
int thread_join(int tid) {
    ...
    // check if there is a thread with the specified TID 
    if (!(0 < tid && tid < NTHR && thrtab[tid] != NULL)) {
        return -1;
    }

    cthr = thrtab[tid]; 

    // check if the calling thread is not the parent of the specific thread
    if (cthr->parent != CURTHR) {
        return -1;
    }

    trace("%s() in %s waiting for child %d", __func__, CURTHR->name, tid);

    // check if child has already exited
    if (cthr->state == THREAD_EXITED) {
        // release the resources and return
        recycle_thread(tid);
        return tid;
    }

    // wait for child to exit then return child TID
    condition_wait(&CURTHR->child_exit);
...
/* wakes up all the threads waiting for a given condition 
iterates through the condition waitlist and wakes up every thread */
...
void condition_broadcast(struct condition * cond) {
    ...
    // iterate through the conditions waitlist and wake up each thread
    while(!tlempty(&cond->wait_list)) {
        // remove the condition from the wait_list
        thr = tlremove(&cond->wait_list);

        // change its state from waiting to ready
        set_thread_state(thr, THREAD_READY);

        // place it in the ready-to-run list
        tlinsert(&ready_list, thr);
    }

    // clear the conditions waiting list
    // tlclear(&cond->wait_list);
}
...
if the thread is still runnable places it back into the ready list
if ready to run list is empty switch to idle thread to soak up all 
the cpu cycles */
void suspend_self(void) {
    ...
    if (CURTHR->state == THREAD_RUNNING) {
        // if thread is still runnable place it back in the list
        set_thread_state(CURTHR, THREAD_READY);
        tlinsert(&ready_list, CURTHR);
    }

    // get the next thread to run from the head of the list
    // check if the ready list is empty
    if (tlempty(&ready_list)) {
        // if no threads available switch to idle thread
        next_thr = &idle_thread;
    } else {
        // else remove an element from the ready list and use that
        next_thr = tlremove(&ready_list);
    }

    // switch to the next ready_to_run thread
    set_thread_state(next_thr, THREAD_RUNNING);

    // switch context
    _thread_swtch(next_thr);

    // panic("suspend_self() returned in exited thread");
}
